...
if(isset($_GET['getinfo']) && preg_match("/^[0-9]{1,10}$/", $_GET['getinfo'])) {
	...
	if(is_array($rs)) {
		// http://admin:admin@192.168.33.20:7500/api/serverinfo
		// {
		// 	"version": "0.60.0",
		// 	"bindPort": 7000,
		// 	"vhostHTTPPort": 19080,
		...
		if(isset($hs['status']) && $hs['status'] == 200) {
			...
			$tf_in  = SakuraPanel\Utils::getFormatTraffic($js['totalTrafficIn']);
			$tf_out = SakuraPanel\Utils::getFormatTraffic($js['totalTrafficOut']);
			echo <<<EOF
<h4>{$rs['name']} <small>节点信息</small></h4>
<hr>
<table style="width: 100%;" class="sinfotable">
	<tr><th>服务端版本</th><td>{$js['version']}</td></tr>
	<tr><th>监听端口</th><td>{$js['bindPort']}</td></tr>
	<tr><th>HTTP 监听端口</th><td>{$js['vhostHTTPPort']}</td></tr>
	<tr><th>HTTPS 监听端口</th><td>{$js['vhostHTTPSPort']}</td></tr>
	<tr><th>总共入网流量</th><td>{$tf_in}</td></tr>
	<tr><th>总共入网流量</th><td>{$tf_out}</td></tr>
	<tr><th>连接数量</th><td>{$js['curConns']}</td></tr>
	<tr><th>客户端数量</th><td>{$js['clientCounts']}</td></tr>
	<tr><th>节点域名</th><td>{$js['subdomainHost']}</td></tr>
	<tr><th>最大代理数量</th><td>{$js['maxPoolCount']}</td></tr>
	<tr><th>最大端口数量</th><td>{$js['maxPortsPerClient']}</td></tr>
</table>
EOF;
			exit;
		} else {
...
if(isset($_GET['gettraffic']) && preg_match("/^[0-9]{1,10}$/", $_GET['gettraffic']) && in_array($_GET['type'], ["tcp", "udp", "http", "https", "stcp"])) {
	...
	if(is_array($rs)) {
		...
		if(isset($hs['status']) && $hs['status'] == 200) {
			...
			echo '<tr><th>隧道名称</th><th>所属用户</th><th>上次启动时间</th><th>上次退出时间</th><th>连接数量</th><th>今日流量 (↓/↑)</th><th>当前状态</th></tr>';
			// echo json_encode($js);
			foreach($js['proxies'] as $proxy) {
				...
				echo "<td>{$proxy['lastStartTime']}</td>";
				echo "<td>{$proxy['lastCloseTime']}</td>";
				echo "<td>{$proxy['curConns']}</td>";
				$tf_in  = SakuraPanel\Utils::getFormatTraffic($proxy['todayTrafficIn']);
				$tf_out = SakuraPanel\Utils::getFormatTraffic($proxy['todayTrafficOut']);
