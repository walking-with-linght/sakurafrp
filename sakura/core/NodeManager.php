...
class NodeManager {
	...
	public function closeClient($node, $token)
	{
		...
		if($ninfo) {
			// echo "http://admin:{$ninfo['admin_pass']}@{$ninfo['ip']}:{$ninfo['admin_port']}/api/client/stop/{$token}";
			// exit;
			// http://admin:admin@192.168.33.20:7500/api/client/stop/46414e6574f2969e
	...
	{
		return Database::toArray(Database::search("nodes", Array("group" => "{$group};", "status" => 200)));
	}
