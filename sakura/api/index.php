...
define("API_TOKEN", "wobushitoken9527");
				...
				if(Regex::isLetter($_GET['user']) && strlen($_GET['user']) == 16) {
					...
					if($rs) {
						if($proxyType == "tcp" || $proxyType == "udp") {
							...
							if(isset($_GET['customdomains']) || isset($_GET['subdomain'])) {
								// 目前只验证域名和子域名
								$domain    = $_GET['customdomains'] ?? "null";
								$subdomain = $_GET['subdomain'] ?? "null";
								$username  = $rs['username'];
								$domain    = $domain;
								$subdomain = $subdomain;
								$domainSQL = (isset($_GET['customdomains']) && !empty($_GET['customdomains'])) ? ["domain" => $domain] : ["subdomain" => $subdomain];
								$querySQL  = [
									"username"   => $username,
									"proxy_type" => $proxyType,
									"node"       => $switchNode
