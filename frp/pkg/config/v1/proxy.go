...
func (c *ProxyBaseConfig) MarshalToMsg(m *msg.NewProxy) {
	...
	// fmt.Println("MarshalToMsg--这里是客户端才会走到这里 懂了，客户端pack，服务器在下面unpack")
	// fmt.Println(m)
}
...
func (c *ProxyBaseConfig) UnmarshalFromMsg(m *msg.NewProxy) {
	...
	//John_San 默认限速1MB,服务器限速
	m.BandwidthLimit = "1MB"
	m.BandwidthLimitMode = "server"
	if m.BandwidthLimit != "" {
	...
	// fmt.Println("UnmarshalFromMsg--服务器打印")
	// fmt.Println(c)
}
