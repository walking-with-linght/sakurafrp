...
func PrintHexAndDecFields(v interface{}) {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			fmt.Printf("%s: 十六进制: %x, 十进制: %d\n", fieldName, field.Int(), field.Int())
		case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uintptr:
			fmt.Printf("%s: 十六进制: %x, 十进制: %d\n", fieldName, field.Uint(), field.Uint())
		case reflect.Ptr:
			fmt.Printf("%s: 指针地址: %p (十六进制), 十进制: %d\n", fieldName, field.Pointer(), field.Pointer())
		default:
			fmt.Printf("%s: %v\n", fieldName, field)
		}
	}
