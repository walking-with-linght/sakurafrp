...
import (
	...
	// httpclient "github.com/fatedier/frp/pkg/util/http"
)
...
// 	fmt.Println("token--VerifyLogin auth.token=服务器的token")
// 	// fmt.Println(auth.token)
// 	util.PrintHexAndDecFields(auth)
// 	fmt.Println(m.Metas)
// 	var url string = auth.token + fmt.Sprintf("/john_san_api?action=%s&user=%s&token=%s&timestamp=%d", "checktoken", m.User,m.PrivilegeKey, time.Now().Unix())
// 	fmt.Println(url)
// 	getResponse, err := httpclient.HttpGet(url)
// 	if (err != nil){
// 		return fmt.Errorf("John_San checktoken Net error")
// 	}
// 	if (getResponse != "success"){
// 		return fmt.Errorf("John_San checktoken error")
// 	}
// 	return nil
// }
