...
func (ctl *Control) RegisterProxy(pxyMsg *msg.NewProxy) (remoteAddr string, err error) {
	...
	fmt.Println("RegisterProxy 注册代理")
	pxyConf, err = config.NewProxyConfigurerFromMsg(pxyMsg, ctl.serverCfg)
	if err != nil {
		fmt.Println("RegisterProxy 注册代理 失败1")
		return
	}
	//新增代理检查
	// s, err := api.NewService(ctl.serverCfg.John_San_Api)
	ok, err := api.CheckProxy(ctl.serverCfg.John_San_Api, ctl.loginMsg.User, pxyConf, pxyMsg, ctl.serverCfg.John_San_Token)
	fmt.Println("RegisterProxy 注册代理 -------")
	if err != nil {
		fmt.Println("RegisterProxy 注册代理 失败2")
		return remoteAddr, err
	}

	if !ok {
		fmt.Println("RegisterProxy 注册代理 失败3")
