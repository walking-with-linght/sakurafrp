...
func (svr *Service) registerRouteHandlers(helper *httppkg.RouterRegisterHelper) {
	...
	//http://admin:admin@192.168.33.20:7500/api/proxy/http
	subRouter.HandleFunc("/api/proxy/{type}", svr.apiProxyByType).Methods("GET")
	//http://admin:admin@192.168.33.20:7500/api/proxy/http/46414e6574f2969e.test
	subRouter.HandleFunc("/api/proxy/{type}/{name}", svr.apiProxyByTypeAndName).Methods("GET")
	//根据名字获取流量 user.name http://admin:admin@192.168.33.20:7500/api/traffic/46414e6574f2969e.test
	subRouter.HandleFunc("/api/traffic/{name}", svr.apiProxyTraffic).Methods("GET")
	// 这应该是清理离线代理的内存  http://admin:admin@192.168.33.20:7500/api/proxies?status=offline
...
func (svr *Service) ApiCloseClient(w http.ResponseWriter, r *http.Request) {
	...
	user := params["user"]
	defer func() {
		log.Infof("Http response [/api/client/close/{user}]: code [%d]", resp.Status)
	}()
	log.Infof("Http request: [/api/client/close/{user}] %#v", user)
	err := svr.CloseUser(user)
	if err != nil {
		resp.Status = 404
		resp.Msg = err.Error()
