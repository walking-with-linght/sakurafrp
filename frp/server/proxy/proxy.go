...
func NewProxy(ctx context.Context, options *Options) (pxy Proxy, err error) {
	...
	// }
	// 准备从这里取数据 
	// fmt.Println("是否是默认限速1MB",configurer.GetBaseConfig().Transport.BandwidthLimit)
	// fmt.Println("John_San_Limit是否设置成功",options.UserInfo.John_San_Limit)
	// // options.UserInfo.John_San_Limit = "1MB"
	// configurer.GetBaseConfig().Transport.BandwidthLimit, _ = types.NewBandwidthQuantity("3MB") //也可以这样填
	// fmt.Println("可以设置吗",configurer.GetBaseConfig().Transport.BandwidthLimit)
	limiter = rate.NewLimiter(rate.Limit(float64(limitBytes)), int(limitBytes))
	...
	fmt.Println("最终限速=========", configurer.GetBaseConfig().Transport.BandwidthLimit)
	return pxy, nil
}
